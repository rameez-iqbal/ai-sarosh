<footer id="footer" data-aos="fade-up">
        <div class="container">
            <div class="row justify-content-between">
                <div class="col-md-4">
                    <img src="{{ asset('app-assets') }}/images/frontend/logo.svg" class="logo" alt="">
                    <span class="al-sarosh-heading">Al-Sarosh</span>
                    <p class="text-white about-al-sarosh">AI-Sarosh is a knowledge hub for responsible use of Artificial Intelligence for Sexual, Reproductive and Maternal Health in South Asia.</p>
                </div>
                <div class="col-md-2">
                    <h6 class="text-white fw-bolder">Links</h6>
                    <ul class="links d-flex flex-column">
                        <li class="text-white"><a href="{{route('homepage')}}">Al-Sarosh</a>
                            <img src="{{ asset('app-assets') }}/images/frontend/arrow_right.svg" class="" width="9" height="9" alt="" >
                        </li>
                        <li class="text-white"><a href="{{route('project')}}">Our Work</a>
                            <img src="{{ asset('app-assets') }}/images/frontend/arrow_right.svg" class="" width="9" height="9" alt="" >
                        </li>
                        <li class="text-white"><a href="{{route('aboutUs')}}">About Us</a>
                            <img src="{{ asset('app-assets') }}/images/frontend/arrow_right.svg" class="" width="9" height="9" alt="" >
                        </li>
                        <li class="text-white"><a href="{{route('team')}}">Our Team</a>
                            <img src="{{ asset('app-assets') }}/images/frontend/arrow_right.svg" class="" width="9" height="9" alt="" >
                        </li>
                        <li class="text-white"><a href="{{route('contact-us')}}">Contact Us</a>
                            <img src="{{ asset('app-assets') }}/images/frontend/arrow_right.svg" class="" width="9" height="9" alt="" >
                        </li>
                        <li class="text-white"><a href="{{route('library')}}">Library</a>
                            <img src="{{ asset('app-assets') }}/images/frontend/arrow_right.svg" class="" width="9" height="9" alt="" >
                        </li>
                    </ul>
                </div>
                <div class="col-md-3 align-contact-us-section">
                    <h6 class="text-white mb-0 fw-bolder">Contact Us</h6>
                    <p class="para mb-0">aisarosh@phcglobal.org</p>
                    <a href="tel:+9234122345" class="fw-normal text-white text-decoration-none">Phone: (+92) 34122345</a>
                    <p class="para mb-0">Office# 241, Near Askari bank, Bahadurabad, Karachi, Sindh, Pakistan</p>
                    <div class="social-icons d-flex justify-content-center justify-content-md-end">
  <img src="{{ asset('app-assets/images/frontend/fb.svg') }}" data-src="{{ asset('app-assets/images/frontend/fb.svg') }}" alt="Facebook">

                        <img src="{{ asset('app-assets') }}/images/frontend/insta.svg" data-src="{{ asset('app-assets') }}/images/frontend/insta.svg" alt="">
                        <img src="{{ asset('app-assets') }}/images/frontend/youtube.svg" data-src="{{ asset('app-assets') }}/images/frontend/youtube.svg" alt="">
                        <img src="{{ asset('app-assets') }}/images/frontend/linkedin.svg" data-src="{{ asset('app-assets') }}/images/frontend/linkedin.svg" alt="">
                    </div>
                </div>

            </div>
            <div class="row text-center footer-al-sarosh">
                <div class="col-md-12">
                    <p class="para">© AI-Sarosh {{ date('Y') }}</p>
                </div>

            </div>
    </div>
</footer>